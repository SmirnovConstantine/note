...
def url_pars():
    ...
    urls = Note.objects.all().filter(is_parse=False)
    return urls


def work_pars():
    """" Парсер для заметок пользователей. """
    b_urls = url_pars()  # передаем список необработанных заметок, url-заметок
    ...
    for b_url in b_urls:
        ...
        if request.status_code == 200:
            ...
            if image[:4] == 'http' or image[:5] == 'https':
                ...
                new_note = Note.objects.get(url=b_url)
                new_note.title = title
                new_note.description = descr
                new_note.image = filename
                new_note.is_parse = True
                new_note.save()
